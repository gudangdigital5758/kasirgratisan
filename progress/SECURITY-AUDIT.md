...
| SEC-008 | P3 | RLS cloud_team_members + pin_hash | PARTIALLY VERIFIED → **FIX REPO READY, PROD-PENDING (2026-08-19)** | Policy RLS: member hanya membaca baris miliknya (`user_id=auth.uid()`), owner semua baris store → **tidak ada bocor silang antar-member**. Namun `pin_hash` masih SELECT-able client (anon/authenticated) pada baris yang diizinkan. Fix: migrasi `20260819000000_sec8_restrict_pin_hash_column.sql` → REVOKE insert/update/select pin_hash dari anon/authenticated (worker service_role tak terpengaruh). **Belum di-apply ke prod (butuh approval db push)** | Hash PIN ter-expose client pada baris yang diizinkan | Push migrasi ke prod setelah approval | `20260819000000_sec8_restrict_pin_hash_column.sql` | MEDIUM |
