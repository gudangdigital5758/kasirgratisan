...
| SEC-005 | P3 | Sesi tim | **RESOLVED (2026-08-19)** | Token sesi → `token_hash` SHA-256 (kolom plaintext di-drop); lookup bearerAuth via hash; `POST /api/team/logout` (revoke); eager cleanup expired saat login; RLS + revoke DML anon/authenticated. Diverifikasi prod: kolom `token` hilang, grants client kosong, RLS on. Test `tests/team-session.test.ts`. Catatan: push migrasi A+B ter-apply bersamaan sebelum deploy → jendela ±2-3 menit auth sesi tim error (ditutup deploy segera; tanpa data loss — 0 sesi aktif saat migrasi) | Token dicuri → sesi aktif tanpa revoke cepat | (SELESAI) hash + revoke endpoint + cleanup | `lib/session.ts`, `routes/team.ts`, `index.ts`, migrasi `2026081910/110000` | MEDIUM |
| SEC-006 | P3 | Info disclosure | **RESOLVED (2026-08-19)** | `/health` publik di production kini hanya subset aman (`ok/service/domain/time`); detail provider/config via `?full=1` (tooling internal; `cloud-check.mjs` diupdate). Non-production tetap penuh. Test `tests/health.test.ts` | Info disclosure minor | (SELESAI) subset aman + opt-in detail | `index.ts`, `scripts/cloud-check.mjs`, `tests/health.test.ts` | HIGH |
...
| SEC-011 | P2 | Cron manual | **FIX REPO READY, PROD-PENDING (2026-08-19)** | Manual `/api/cron/*` kini mendukung HMAC-SHA256 `timestamp.method.path` (5 menit anti-replay) memakai `CRON_HMAC_SECRET`; setelah key dipasang header `WEBHOOK_SECRET` lama ditolak. Scheduled Worker cron tidak berubah. Test 4 HMAC cases. | Secret statis di header; replay/leak → trigger cron | Install `CRON_HMAC_SECRET`, deploy, lalu pindahkan caller manual ke header HMAC | `lib/cron-auth.ts`, `routes/cron.ts`, `tests/cron-hmac.test.ts` | MEDIUM |
