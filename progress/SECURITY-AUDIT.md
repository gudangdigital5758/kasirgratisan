...
| SEC-005 | P3 | Sesi tim | **FIX REPO READY, PROD-PENDING (2026-08-19)** | Token sesi kini disimpan sebagai `token_hash` (SHA-256 hex; migrasi A backfill + revoke DML anon/authenticated; migrasi B hapus kolom plaintext). Lookup bearerAuth via hash; endpoint `POST /api/team/logout` (revoke); eager cleanup sesi expired saat login. Test `tests/team-session.test.ts` (3) | Token dicuri → sesi aktif tanpa revoke cepat | Push migrasi A → deploy → push migrasi B (urutan wajib) | `lib/session.ts`, `routes/team.ts`, `index.ts`, migrasi `2026081910/110000` | MEDIUM |
