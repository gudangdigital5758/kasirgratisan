...
- **Status prod**: **PRODUCTION FUNCTION VERIFIED (2026-08-18)** — `pg_get_functiondef` membuktikan `raw_json` tanpa pola `raw = raw`; migrasi fix `20260817020000` di-push + tercatat di `supabase_migrations`. Integration local 3× (embedded PG18) + CI GREEN.
...
| FUL-006 | P2 | Monitoring | **PARTIALLY RESOLVED (2026-08-19)** | Cron `/api/cron/billing-health` + `flagBillingAnomalies`: alert `billing.payment_completed_without_subscription` (24 jam) & `billing.legacy_pin_hash`; stale-pending alert sudah ada. Alerting provider/webhook failure (kanal notifikasi internal, mis. email/WA admin) belum — butuh desain kanal | Tidak ada alerting otomatis untuk kegagalan fulfillment/webhook | Kanal notifikasi internal (email/WA admin) + integrasi scan | `lib/lifecycle.ts`, `routes/cron.ts`, `tests/billing-health.test.ts` | MEDIUM |
