...
## FUL-009 + FUL-008 CLOSE + RECONCILIATION (2026-08-19)
- FUL-009 **COMPLETE — NO PAID UNFULFILLED PAYMENT**. Evidence provider resmi (dashboard SumoPod, tab *Customer Payments* Live): kedua payment "ambiguous" = **Cancelled** (`00121cc8`, `9c02932e`); 7 PENDING fixture = **Expired**; satu-satunya **Completed** provider = `a269c51d` (entitlement sudah diberikan). Tidak ada pembayaran yang dibayar tapi belum ber-entitlement. Exposure finansial final: **Rp0**.
- FUL-008 exposure tertutup: confirmed Rp0, potential/unknown Rp0 (semua unpaid).
- Reconciliation (approved): 9 payment PENDING → **FAILED** di produksi via logika cron `flagStalePendingPayments` + `AUTO_FAIL_STALE_PENDING=true` (SQL ekivalen atomik; audit trail 9x `payment.stale_pending` di platform_events; raw di-merge + marker `stalePending.autoFailed=true`; 2 COMPLETED tidak tersentuh). `21cd4b6c` (E2E test) tetap COMPLETED — order test expired di provider, dampak hanya fixture store (tanpa uang nyata), didokumentasikan.
- Worker produksi di-deploy ulang ke `94121a92` (memuat hardening c206293 + FUL-010 removal 837444c + fix merge raw 98087d6).
- **AUTO_FAIL_STALE_PENDING = `false` (alert-only) sejak 2026-08-19** — menunggu verifikasi E2E webhook riil (minggu depan). 9 payment yang sudah di-FAILED tetap FAILED (tuntas). Cron harian tetap menulis alert `payment.stale_pending` tanpa auto-FAIL; diaktifkan kembali (`true`) setelah webhook E2E terverifikasi.
- FUL-001/007: tetap FIXED+VERIFIED. Phase 0 gate: NOT READY (sisa SEC-005..012, BILL, FUL-003/004/006, §11, deploy-verify webhook end-to-end real).
