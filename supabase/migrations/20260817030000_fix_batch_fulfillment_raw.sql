-- FUL-007 (2026-08-17): fix ambiguity `raw` di fulfill_cloud_payment_batch.
-- Pola identik FUL-001: variabel plpgsql `raw jsonb` vs kolom payments.raw di
-- `update public.payments ... set raw = raw, ...` -> PG15 'column reference raw
-- is ambiguous'. Fix: rename variabel raw -> raw_json. Behavior tidak berubah.
-- Idempotent (create or replace); grant/revoke: migrasi asli tidak punya
-- (default EXECUTE PUBLIC) - dipertahankan.
...
begin
  ...
  if pay.status = 'COMPLETED' and coalesce(raw_json->>'batchFulfilled', 'false') = 'true' then
    return jsonb_build_object(
      'alreadyDone', true,
      'fulfilledCount', jsonb_array_length(items),
      'periodEnd', raw_json->>'periodEnd',
      'isLifetime', coalesce(lower(raw_json->>'isLifetime') = 'true', false)
    );
  end if;
